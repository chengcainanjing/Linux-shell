...
# History:
# 2019.07.16 chengcai	First release
#
...
# delete the environment variable
    ...
    ASK_COUNT=0
#
    while [ -z "$ANSWER" ] #while no answer is given, keep asking. -z 检查是否为0
    do
...
#
# Answer 60 seconds to answer before time-out
        read -t 60 ANSWER
    done
#
...
#    if user answers anything but "yse", exit script
#
...
0)  # Processes running for this User Account
    ...
    y|Y|yes|YES|Yes|yEs|yeS|YEs|yES)            # if user answer "yes",
        ...
        do
            # obtain PID
            USER_PID=$(echo $USER_PROCESS_REC | cut -d " " -fi )
            kill -9 $USER_PID
...
#
REPORT_DATA=`data +%y%m%d`
REPORT_FILE=$USER_ACCOUNT"_Files_"$REPORT_DATA".rpt"
#
